...
docker build -t courier-mta-ssl -f Dockerfile-mta-ssl .
# MSA image, for allowing authenticated users to submit email delivery to other mail servers
docker build -t courier-msa -f Dockerfile-msa .
# IMAPD-SSL image, for checking mail via IMAP
docker build -t courier-imapd-ssl -f Dockerfile-imapd-ssl .
# And a container for the courierd
docker build -t courierd -f Dockerfile-courierd .
